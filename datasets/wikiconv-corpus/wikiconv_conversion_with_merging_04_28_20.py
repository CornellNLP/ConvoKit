...
def main():
    ...
    utterances_before_output = 5*(10 ** 5) #number of utterances before output is generated to limit memory consumption, initalized to half a million
    ...
    for input_filename in os.listdir(data_directory_input):
        if ((input_filename.endswith(".jsonlist") and (not(input_filename in input_files_read)))):
            ...
            individual_utterance_list, conversation_individual_dict = create_utterances_from_individual_file(final_input_path)
            ...
            if (len(complete_utterance_list)> utterances_before_output):
                ...
                for key, value in dictionary_of_year_lists.items():
                    set_meta_dump_corpus(value, master_conversation_level_dict, data_directory_intermediate, 
    ...
    dictionary_of_year_lists = separate_by_year(complete_utterance_list)
    for key, value in dictionary_of_year_lists.items():
        set_meta_dump_corpus(value, master_conversation_level_dict, data_directory_intermediate, 'conv_corpus_year_'+str(key)+'_data_split_'+str(count_of_data_splits))
...
def is_empty_utterance(utterance):
    ...
    text_is_empty = not utterance.text or utterance.text.strip() == "" or utterance.text.strip() == '-'
...
def set_meta_dump_corpus(complete_utterance_list,master_conversation_level_dict, data_directory_output, corpus_name):
    filtered_utterance_list = [u for u in complete_utterance_list if not is_empty_utterance(u)]
    
    print(f"Filtered {len(complete_utterance_list) - len(filtered_utterance_list)} empty utterances")
...
def create_utterances_from_individual_file(name_of_file_to_convert):
    ...
    with open(name_of_file_to_convert, "r") as f:
        for line in f:
            ...
            conversation_meta_dict = {'page_id':page_id, 'page_title': page_title, 'page_type': page_type}
            ...
            if (len(comments) > 0):
                for comment_number in range(len(comments)):
                    ...
                    utterance_val, order_identification_val = reformat_comment(comment_val, section_header_value)

                    dict_of_utterances[utterance_val.id] = utterance_val
                    list_of_order_identification_values.append(order_identification_val)

                correct_order_of_comments = correct_comment_order(list_of_order_identification_values)


                final_utterance_list = final_dict(dict_of_utterances, correct_order_of_comments)
                flat_utterance_list = [utterance_sub for sublist in final_utterance_list for utterance_sub in sublist]
                    ...
                    for utterance in finalized_utterance_list:
                        ...
                        if (current_comment_order_id['parent_id'] == utterance.id):
                            ...
                            if (utterance.meta['original'] is None):
                                # print("hm")
                                utterance.meta['original'] = utterance_to_dict(original_utterance_value)
                        check_action_lists(utterance, current_comment_order_id, dict_of_utterances, current_comment_order_id_type)
                        convert_utterance_values_to_dict(utterance, current_comment_order_id, dict_of_utterances, current_comment_order_id_type)
...
def create_utterance_list(list_of_comments):
    ...
    for individual_comment_list in list_of_comments: 
        ...
        speaker_value = Speaker(id = speaker_dict['speaker'], meta = {'speaker_id': speaker_dict['speaker_id']})

        #Construct the utterance value of type Utterance Class
        utterance_value = Utterance(ind_com['id'], speaker_value, ind_com['root'], ind_com['reply-to'], ind_com['timestamp'], ind_com['text'], None, meta = ind_com['meta'])
        final_list_of_utterances.append(utterance_value)
    return final_list_of_utterances


#Check within the lists
def check_action_lists(utterance, current_comment_order_id, dict_of_utterances, current_comment_order_id_type):
    modification_list = utterance.meta['modification']
    deletion_list = utterance.meta['deletion']
    restoration_list = utterance.meta['restoration']
    comment_to_append = dict_of_utterances[current_comment_order_id['id']]
    if (len(modification_list)>0):
        for utterance_val in modification_list:
            check_id_add(utterance, utterance_val, current_comment_order_id, current_comment_order_id_type, comment_to_append)
    if (len(deletion_list)>0):
...
def utterance_to_dict(utterance_val):
    ...
    if (type(utterance_val) == Utterance):
        ...
        dict_rep['speaker'] = {'id':utterance_val.speaker.id, 'speaker_id':utterance_val.speaker.meta['speaker_id']}
...
def check_id_add(utterance, utterance_val, current_comment_order_id, current_comment_order_id_type, comment_to_append):

    if (current_comment_order_id['parent_id'] == get_id_from_utt_or_dict(utterance_val) and check_not_in_list(utterance.meta[current_comment_order_id_type], comment_to_append)):
        utterance.meta[current_comment_order_id_type].append(comment_to_append)
        if (current_comment_order_id_type == 'deletion'):
            rewrite_utterance_deletion(utterance, comment_to_append)
        else:
...
def correct_comment_order(list_of_order_identification_values):
    ...
    for value in list_of_order_identification_values:
        ...
        elif (current_type == 'modification' or current_type == 'deletion' or current_type == 'restoration'):
...
def reformat_comment(json_val, section_header_value):
    ...
    #Build the meta dict 
    meta_dict = {'is_section_header':is_section_header, 'indentation':indentation, 'toxicity':toxicity, 'sever_toxicity':sever_toxicity, 'ancestor_id':ancestor_id, 'rev_id':rev_id, 'parent_id':parent_id, 
            'original': None, 'modification': [], 'deletion': [], 'restoration':[]}

    #Construct the utterance value of type Utterance Class
    utterance_value = Utterance(id = id_val,speaker= speaker_value, conversation_id = root_val, reply_to = reply_to,timestamp = timestamp, text= text, meta = meta_dict)
    order_identification = {'id':id_val, 'parent_id':parent_id, 'type':type_val, 'timestamp': timestamp}
