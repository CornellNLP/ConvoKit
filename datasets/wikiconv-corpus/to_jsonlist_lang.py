...
print_lock = Lock()
stats = {"downloaded": 0, "converted": 0, "failed": 0, "total_conversations": 0}
...
def parse_timestamp(timestamp_str):
    ...
    formats = ["%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d %H:%M:%S UTC", "%Y-%m-%d %H:%M:%S"]
...
def convert_to_jsonlist(input_path, output_path):
    ...
    conversations = defaultdict(lambda: {"comments": [], "authors": set(), "page_info": {}})
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_file = {
            executor.submit(
                download_and_convert_file, file_info, RAW_DIR, OUTPUT_DIR, idx, len(files)
            ): file_info
