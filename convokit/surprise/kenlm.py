...
class Kenlm(LanguageModel):
    ...
    def __init__(self, model_type: str = 'kenlm', **kwargs: Optional[Any]):
        ...
        if self._is_persistent or 'trained_model_filepath' in kwargs:
            ...
            self.__kenlm_model = Kenlm.load_kenlm_from_file(
                kwargs['trained_model_filepath']) if 'trained_model_filepath' in kwargs else None
    ...
    def _get_kenlm_model(self, context_samples: Union[List[List[str]], np.ndarray]) -> kenlm.Model:
        ...
        kenlm_args = [os.path.join(self.__kenlm_bin_path, 'lmplz'), '-o', f'{self._ngram_order}', '--text',
                      train_filename, '--arpa', arpa_filename, '--discount_fallback']
        cmd_return = subprocess.run(kenlm_args, capture_output=False, text=True, stdout=subprocess.DEVNULL,
        ...
        kenlm_args = [os.path.join(self.__kenlm_bin_path, 'build_binary'), 'trie', arpa_filename, model_filename]
        cmd_return = subprocess.run(kenlm_args, capture_output=False, text=True, stdout=subprocess.DEVNULL,
                                    stderr=subprocess.STDOUT)
        if cmd_return.returncode != 0:
            delete_files([model_filename, arpa_filename, train_filename])
    ...
    def cross_entropy(self, target: Union[List[str], np.ndarray], context: Union[List[str], np.ndarray]) -> float:
        ...
        return -self.__kenlm_model.score(' '.join(target))
